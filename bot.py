...
def is_bad(symbol, price):
    ...
    import re
    if not re.match(r'^[A-Z][A-Z0-9]{1,9}USDT$', symbol): return True
    if 0.985 <= price <= 1.015: return True
    return False
...
# ── Signal logic ─────────────────────────────────────────────
# Strategy: MACD cross + RSI zone + VWAP + 1H trend filter
# 1H trend must agree with 15m signal for higher accuracy
...
def get_signal(klines_15m, klines_1h):
    ...
    vol_up  = vols[-1] > vol_avg

    # 1H trend filter — EMA21 direction on 1h
    closes_1h = [float(k[4]) for k in klines_1h]
    ema21_1h  = ema(closes_1h, 21)
    ema50_1h  = ema(closes_1h, 50)
    trend_1h_bull = ema21_1h[-1] > ema50_1h[-1]
    trend_1h_bear = ema21_1h[-1] < ema50_1h[-1]

    # MACD crossover (15m)
    bull_cross = (macd['prev_macd'] <= macd['prev_sig'] and macd['macd'] > macd['signal']) or \
                 (macd['macd'] > macd['signal'] and macd['hist'] > macd['prev_hist'] and macd['hist'] > 0)
    bear_cross = (macd['prev_macd'] >= macd['prev_sig'] and macd['macd'] < macd['signal']) or \
                 (macd['macd'] < macd['signal'] and macd['hist'] < macd['prev_hist'] and macd['hist'] < 0)
    ...
    # Long: all 15m signals + 1h trend confirms up
    long_ok  = bull_cross and above_vwap and long_rsi  and trend_1h_bull
    # Short: all 15m signals + 1h trend confirms down
    short_ok = bear_cross and not above_vwap and short_rsi and trend_1h_bear
    ...
    # Score (max 5 with 1h filter)
    if direction == 'LONG':
        score = sum([bull_cross, above_vwap, long_rsi, vol_up, trend_1h_bull])
        signals = {
            'MACD↑': bull_cross,
            'ABOVE VWAP': above_vwap,
            'RSI OK': long_rsi,
            'VOL↑': vol_up,
            '1H TREND↑': trend_1h_bull,
        }
    else:
        score = sum([bear_cross, not above_vwap, short_rsi, vol_up, trend_1h_bear])
        signals = {
            'MACD↓': bear_cross,
            'BELOW VWAP': not above_vwap,
            'RSI OK': short_rsi,
            'VOL↑': vol_up,
            '1H TREND↓': trend_1h_bear,
        }
...
def format_signal(name, sig):
    ...
    return f"""{emoji} <b>{arrow} {sig['direction']} — {name}/USDT</b>
━━━━━━━━━━━━━━━━━━
📍 Entry:    <b>{fmt(sig['entry'])}</b>
🛑 Stop:     <b>{fmt(sig['sl'])}</b>  (-{sig['pct_risk']}%)
🎯 TP1:      <b>{fmt(sig['tp1'])}</b>  (+{sig['pct_target']}%)
🎯 TP2:      <b>{fmt(sig['tp2'])}</b>
━━━━━━━━━━━━━━━━━━
{sig_line}
━━━━━━━━━━━━━━━━━━
Confidence: {stars} {sig['score']}/5
RSI: {sig['rsi']:.1f}  |  R/R: 2.0
⚡ Confirm on Bybit 15m before entering"""
...
def run_scan():
    ...
    for t in tickers:
        ...
        if float(t.get('priceChangePercent', 0)) == 0: continue
    ...
    for sym in candidates:
        try:
            ...
            if not isinstance(klines_15m, list) or len(klines_15m) < 60:
                continue
            if not isinstance(klines_1h, list) or len(klines_1h) < 60:
                continue
            ...
            time.sleep(0.2)  # be gentle with Binance
    ...
    # Send each signal
    print(f"Sending {len(top_signals)} signals to Telegram...")
    header = f"🔍 <b>SCAN COMPLETE</b> — {datetime.utcnow().strftime('%H:%M UTC')}\n{len(top_signals)} signal(s) found\n"
    send_message(header)
    time.sleep(1)
...
if __name__ == "__main__":
    ...
    send_message(f"🤖 <b>Signal Bot Started</b>\nScanning top 80 coins every {SCAN_INTERVAL_MINUTES} min\nStrategy: MACD + RSI + VWAP + 1H Trend\nTarget win rate: 70%+")
